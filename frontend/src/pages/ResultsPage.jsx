...
// The ring reads out the NCLC level, not the raw score: NCLC is what IRCC acts
// on, and it is the part the rating→score mapping is calibrated to get right.
function ScoreRing({ scores }) {
  const circumference = 2 * Math.PI * 80;
  const offset = circumference - (scorePct(scores.score) / 100) * circumference;
  return (
    <div className="relative w-48 h-48 flex-shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-2xl font-bold text-white">
          {scores.nclc ? `NCLC ${scores.nclc}` : '—'}
        </span>
        <span className="text-xs text-indigo-200">{scores.cefr}</span>
      </div>
    </div>
  );
}

// Per-criterion breakdown. The headline number tells a candidate where they are;
// only this tells them which of the eight criteria to work on next.
function CriteriaTable({ title, accent, criteria }) {
  const rows = (criteria ?? []).filter((c) => c && c.rating != null);
  if (rows.length === 0) return null;

  const barColor = (rating) => {
    if (rating >= 4) return 'bg-secondary';
    if (rating >= 3.5) return 'bg-tertiary-fixed-dim';
    return 'bg-error';
  };

  return (
    <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6 canvas-shadow">
      <h4 className={`font-bold text-sm mb-4 flex items-center gap-2 ${accent}`}>
        ...
        {rows.map((c) => (
          <div key={c.metric} className="space-y-1">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs font-semibold text-on-surface dark:text-slate-200">
                {c.label}
              </span>
            ...
            <div className="w-full h-1.5 bg-outline-variant/20 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${barColor(c.rating)}`}
                style={{ width: `${Math.max(0, Math.min(100, ((c.rating - 1) / 4) * 100))}%` }}
              />
            </div>
            {c.recommendation && (
...
export default function ResultsPage() {
  ...
  // Declarative redirect. Calling navigate() in the render body triggers React's
  // "cannot update a component while rendering a different component" warning.
  if (!evaluation) {
    return <Navigate to="/practice" replace />;
  }

  const scores = safeScores(evaluation);
  const finalScore = scores.score ?? 0;
  ...
  const allCorrections = [
    ...originalsA.slice(0, 2).map((o, i) => ({
      type: 'Section A', color: 'primary', title: 'Correction', original: o, correction: correctionsA[i] ?? '',
    })),
    ...originalsB.slice(0, 2).map((o, i) => ({
      type: 'Section B', color: 'secondary', title: 'Correction', original: o, correction: correctionsB[i] ?? '',
    })),
  ];

  // NCLC 7 (the Express Entry writing bar) is 310/450. Progress is measured
  // against that, not against a perfect 450, because clearing NCLC 7 is what
  // candidates are actually here for.
  const scoreStatus = statusLabel(scores);
  const progressPct = thresholdPct(finalScore);
  const band = bandLabel(scores);
    ...
    <>
      ...
      <header className="mb-10">
        <h2 className="text-3xl font-headline font-extrabold text-primary dark:text-indigo-300 tracking-tight mb-2">
          {scores.expressEntryEligible
            ? 'Excellent travail — vous atteignez le seuil NCLC 7 !'
        ...
        <div className="col-span-12 lg:col-span-8 primary-gradient rounded-xl p-8 flex flex-col md:flex-row items-center justify-between text-white overflow-hidden relative">
          <div className="relative z-10">
            <span className="text-indigo-200 font-label text-xs tracking-widest uppercase mb-2 block">
              Score d'expression écrite
            </span>
            <div className="flex items-baseline space-x-2">
              <span className="text-7xl font-headline font-extrabold">{finalScore}</span>
              <span className="text-2xl text-indigo-300 font-medium">/ {SCORE_MAX}</span>
            </div>
            <p className="mt-2 text-sm text-indigo-200">
              Barème « équivalence ancien score » — la valeur exigée par IRCC
              {band && <span className="text-indigo-300"> · plage NCLC {scores.nclc} : {band}</span>}
            </p>
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-1 text-xs text-indigo-200">
              <span>
                Section A <strong className="text-white">{scores.sectionA?.score ?? '—'}</strong>
                {scores.sectionA?.nclc ? ` · NCLC ${scores.sectionA.nclc}` : ''}
              </span>
              <span>
                Section B <strong className="text-white">{scores.sectionB?.score ?? '—'}</strong>
                {scores.sectionB?.nclc ? ` · NCLC ${scores.sectionB.nclc}` : ''}
              </span>
              <span className="text-indigo-300">Pondération 40 % / 60 %</span>
            </div>
      ...
      <div className="grid grid-cols-12 gap-8 mb-12">
        ...
        <section className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {judgeA.justification && (
              <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6 canvas-shadow border-l-4 border-primary">
                <div className="flex justify-between items-start mb-4">
                  <span className="bg-primary-fixed text-on-primary-fixed px-3 py-1 rounded-full text-xs font-bold">Section A</span>
            ...
            {judgeB.justification && (
              <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6 canvas-shadow border-l-4 border-secondary">
                <div className="flex justify-between items-start mb-4">
                  <span className="bg-secondary-container text-on-secondary-container px-3 py-1 rounded-full text-xs font-bold">Section B</span>
                  <span className="text-xs text-slate-400">Analyse</span>
                </div>
                <p className="text-xs text-on-surface-variant leading-relaxed line-clamp-5">
                  {judgeB.justification}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <CriteriaTable
              title="Section A — détail par critère"
              accent="text-primary"
              criteria={evaluation.taskA?.criteria}
            />
            <CriteriaTable
              title="Section B — détail par critère"
              accent="text-secondary"
              criteria={evaluation.taskB?.criteria}
            />
          </div>
          ...
          {(originalsA.length > 2 || originalsB.length > 2) && (
            <div className="bg-surface-container-lowest dark:bg-slate-800 rounded-xl p-6 canvas-shadow">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {originalsA.length > 0 && (
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-primary mb-2">Section A</p>
                ...
                {originalsB.length > 0 && (
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-secondary mb-2">Section B</p>
                    <div className="space-y-1.5">
                      {originalsB.map((o, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs">
                          <span className="text-error line-through flex-shrink-0">{o}</span>
        ...
        <section className="col-span-12 lg:col-span-4 bg-surface-container dark:bg-slate-800/60 rounded-xl p-8">
          ...
          <div className="space-y-6">
            {[
              { num: '1', title: 'Section A', text: judgeA.recommendation },
              { num: '2', title: 'Section B', text: judgeB.recommendation },
            ]
              .filter((r) => r.text)
              .map((r) => (
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs font-bold text-primary">
              Progression vers NCLC 7 ({EXPRESS_ENTRY_THRESHOLD}/{SCORE_MAX})
            </p>
