...
function SubmissionItem({ submission, onClick }) {
  const scores = safeScores(submission);
  const label = nclcLabel(scores.nclc);
  return (
    <div
    ...
    >
      <div className="flex items-center space-x-5">
        ...
        <div>
          <h4 className="font-bold text-primary dark:text-indigo-300">Expression écrite — Sections A + B</h4>
      ...
      <div className="flex items-center space-x-8">
        <div className="text-right">
          ...
          <p className="text-lg font-headline font-bold text-primary dark:text-indigo-300">
            {scores.score ?? '—'}{' '}
            <span className="text-xs font-normal text-on-surface-variant">/ {SCORE_MAX}</span>
          </p>
          {scores.sectionA?.score != null && scores.sectionB?.score != null && (
            <p className="text-[11px] text-on-surface-variant">
              A {scores.sectionA.score} · B {scores.sectionB.score}
            </p>
          )}
        </div>
    ...
    return (
      <div>
        ...
        <div className="space-y-1">
          {rows.map((c) => (
            <div key={c.metric} className="flex items-baseline justify-between gap-3 text-xs">
              <span className="text-on-surface-variant">{c.label}</span>
              <span className="font-bold text-on-surface dark:text-slate-200 tabular-nums">
        ...
        <div className="sticky top-0 bg-surface-container-lowest dark:bg-slate-900 px-8 py-6 border-b border-outline-variant/20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-3xl font-headline font-extrabold text-primary">
              {scores.score}
              <span className="text-base font-normal text-on-surface-variant"> / {SCORE_MAX}</span>
            </span>
        ...
        <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface-container-low dark:bg-slate-800 rounded-xl p-5 space-y-3 text-sm">
              ...
              {renderCriteria(submission.criteria_a)}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface-container-low dark:bg-slate-800 rounded-xl p-5 space-y-3 text-sm">
              ...
              {renderCriteria(submission.criteria_b)}
...
export default function DashboardPage() {
  ...
  const scoreOf = (s) => safeScores(s).score ?? 0;
  const avgScore = submissions.length
    ? Math.round(submissions.reduce((sum, s) => sum + scoreOf(s), 0) / submissions.length)
    : 0;
  const bestScore = submissions.length ? Math.max(...submissions.map(scoreOf)) : 0;
  // Progress toward the NCLC 7 bar (310/450), not toward a perfect 450 — the
  // threshold is the goal candidates are actually working to.
  const progressPct = submissions.length ? thresholdPct(avgScore) : 0;
  const gaugeOffset = 552.9 - (552.9 * Math.max(0, Math.min(100, progressPct))) / 100;
  const latestScores = latest ? safeScores(latest) : null;
            ...
            <>
              ...
              <div className="flex-1 grid grid-cols-2 gap-6">
                <div className="bg-surface-container-low dark:bg-slate-700/50 p-6 rounded-lg">
                  ...
                  <p className="text-xs text-on-surface-variant mt-1">sur {SCORE_MAX} points</p>
                </div>
                <div className="bg-surface-container-low dark:bg-slate-700/50 p-6 rounded-lg">
                ...
                <div className="bg-surface-container-low dark:bg-slate-700/50 p-6 rounded-lg">
                  ...
                  <p className="text-3xl font-headline font-bold text-primary dark:text-indigo-300">
                    {EXPRESS_ENTRY_THRESHOLD}
                  </p>
                  <p className="text-xs text-on-surface-variant mt-1">NCLC 7 — seuil Entrée express</p>
                </div>
              </div>
            </>
          ) : (
            ...
            {latest ? (
              <>
                ...
                <div className="flex items-end space-x-2">
                  <span className="text-5xl font-headline font-extrabold">{latestScores.score}</span>
                  <span className="text-xl text-indigo-300 font-medium mb-1">/ {SCORE_MAX}</span>
                </div>
              </>
            ) : (
